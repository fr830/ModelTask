...
/**
 * 应急调度模式下的输入文件组装类
 * 应急模式下分水计划取当前流量值，污染渠段下游所有的分水口与退水闸的分水量设置为0，即关闸
 *        节制闸的闸前闸后水位取当前实时值
 * @author Frank
 *
 */
public class EmergentModelInputFileWriter implements ModelInputFileWriter {
	/** 模型输入文件	 */
	private String modelInputFile = null; 
	
	public EmergentModelInputFileWriter(String filename) {
			...
			if (lines != null && lines.size() > 0) {
				...
				while (rowIndex < lines.size()) {
					...
					case 2: // 第3行 1 6 98 1000 0 600 600 0 0 1
						...
						word2[4] = String.valueOf(param.getReactTime());
						word2[5] = String.valueOf(param.getUpTime());
						word2[6] = String.valueOf(param.getDownTime());
						word2[7] = String.valueOf(param.getInterval());
						word2[8] = String.valueOf(param.getEmerControlType());
						word2[9] = String.valueOf(param.getOperationMode());
